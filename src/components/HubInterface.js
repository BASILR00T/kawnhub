...
const BentoCard = ({ children, className, isPreview, href }) => { // ❗ Step 2: Accept 'href'
    const Tag = isPreview ? 'div' : Link; // Use Link component if not in preview
    return (
        <Tag href={isPreview ? null : href} className={`group relative flex flex-col justify-between overflow-hidden rounded-2xl border border-border-color bg-surface-dark p-6 transition-transform duration-300 ease-in-out hover:-translate-y-1 ${className}`}>
    ...
    return (
        <div className="mx-auto max-w-6xl p-6">
            ...
            <main className="grid grid-cols-6 auto-rows-[220px] gap-4">
                ...
                <BentoCard className="col-span-6 md:col-span-2" isPreview={isPreview} href="#">
                    <div className='flex-grow'><h3 className="text-xl font-bold">أحدث إضافة</h3><p className="mt-2 font-semibold">تأمين منافذ السويتش</p><p className="mt-1 text-sm text-text-secondary">شرح مفصل لآلية عمل Port Security.</p></div>
                    <span className="self-start mt-4 font-bold text-primary-blue no-underline flex items-center gap-2">اقرأ الشرح <ArrowIcon /></span>
                </BentoCard>
                
                {/* ❗ Step 3: Add the correct 'href' to each card */}
                <BentoCard className="col-span-3 md:col-span-2" isPreview={isPreview} href="/materials/pc-config">
                    <div className="text-text-secondary group-hover:text-primary-blue mb-4"><ComputerIcon /></div>
                    <div className="flex-grow"><h3 className="text-lg font-bold">PC Config & Troubleshooting</h3><p className="text-sm text-text-secondary mt-2">أوامر فحص النظام وإصلاح مشاكل ويندوز.</p></div>
                    <div className="self-end text-text-secondary opacity-0 group-hover:opacity-100"><ArrowIcon /></div>
                </BentoCard>
                <BentoCard className="col-span-3 md:col-span-2" isPreview={isPreview} href="/materials/nos">
                    <div className="text-text-secondary group-hover:text-primary-blue mb-4"><LinuxIcon /></div>
                    <div className="flex-grow"><h3 className="text-lg font-bold">Network Operating Systems</h3><p className="text-sm text-text-secondary mt-2">إدارة المستخدمين والصلاحيات في Linux.</p></div>
                    <div className="self-end text-text-secondary opacity-0 group-hover:opacity-100"><ArrowIcon /></div>
                </BentoCard>
                <BentoCard className="col-span-3 md:col-span-2" isPreview={isPreview} href="/materials/network-2">
                    <div className="text-text-secondary group-hover:text-primary-blue mb-4"><NetworkIcon /></div>
                    <div className="flex-grow"><h3 className="text-lg font-bold">Network 2</h3><p className="text-sm text-text-secondary mt-2">بروتوكولات التوجيه وإعدادات السويتش.</p></div>
                    <div className="self-end text-text-secondary opacity-0 group-hover:opacity-100"><ArrowIcon /></div>
                </BentoCard>
            </main>
        </div>
