'use client'; // الخطوة 1: نحول المكون إلى Client Component
...
export default function HubInterface({ isPreview = false }) {
    ...
    // Fetch all materials once when the component loads
    useEffect(() => {
        if (isPreview) return;
        ...
        const getMaterials = async () => {
            ...
            setFilteredMaterials(materialsList); // Initially, show all materials
            setIsLoading(false);
        };
        getMaterials();
    }, [isPreview]);

    // Effect to filter materials whenever the search query changes
    useEffect(() => {
        if (!searchQuery) {
            setFilteredMaterials(allMaterials); // If search is empty, show all
            return;
        }
        ...
        <div className="mx-auto max-w-6xl p-6">
            ...
            <main className="grid grid-cols-6 auto-rows-[220px] gap-4">
                <BentoCard className="col-span-6 md:col-span-4" isPreview={isPreview} href="#">
                    ...
                    {/* The Search Input is now connected to our state */}
                ...
                {/* ... "Latest Addition" Card remains the same ... */}
                ...
                {/* We now map over 'filteredMaterials' instead of 'materials' */}
                {isLoading ? (
