...
export default function SupportClient({ initialSupporters = [] }) {
  ...
  const handleSubmit = async (formData) => {
    ...
    console.log("Submitting form data:", Object.fromEntries(formData)); // DEBUG
    const result = await sendMessage(formData);
    console.log("Server action result:", result); // DEBUG
    ...
    <div className="mx-auto max-w-6xl p-6">
      ...
      <div className="mb-16"> {/* ✅ تمت إزالة max-w-3xl ليأخذ العرض الكامل */}
        <div className="bg-surface-dark border border-border-color rounded-2xl p-8 relative overflow-hidden shadow-lg">
          ...
          <form id="supportForm" onSubmit={(e) => { e.preventDefault(); handleSubmit(new FormData(e.target)); }} className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <select name="type" defaultValue="suggestion" className="w-full rounded-xl bg-background-dark border border-border-color p-3.5 text-sm focus:border-primary-blue outline-none transition-all">
