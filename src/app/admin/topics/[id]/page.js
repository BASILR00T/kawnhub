...
const BlockPreview = ({ block }) => { 
    ...
    switch (block.type) {
        case 'paragraph':return (<p className="..." dangerouslySetInnerHTML={{ __html: sanitize(block.data.en) }} />);
        case 'note':return (<div className="..." dangerouslySetInnerHTML={{ __html: sanitize(block.data.en) }} />);
        ...
        case 'videoEmbed': if (!block.data.url) return <div className="my-6 text-center text-text-secondary">[Video Preview]</div>; return (<div className="my-6"><div className="aspect-w-16 aspect-h-9"><iframe src={block.data.url} title={block.data.caption} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen className="w-full h-full rounded-lg"></iframe></div>{block.data.caption && <p className="text-center text-xs text-text-secondary mt-2">{block.data.caption}</p>}</div>);
        default: return null;
    }
};
...
export default function EditTopicPage({ params }) {
    ...
    const topicId = params.id; // في Next.js 15 قد تحتاج لـ (await params).id لكن سنبقيها هكذا للكود الحالي
