...
export default function EditTopicPage({ params }) {
    ...
    // Fetch materials and the specific topic's data
    useEffect(() => {
        if (!topicId) return;
        const fetchData = async () => {
            ...
            // Fetch the specific topic to edit
    ...
    // --- Content Block Functions (UPDATED) ---
    const addContentBlock = (type) => {
        let newBlock;
        switch(type){
            case 'subheading': //  تمت إضافة الحالة هنا
    ...
    const handleContentChange = (index, field, value) => { const updatedContent = [...content]; if (typeof updatedContent[index].data === 'object' && updatedContent[index].data !== null) { updatedContent[index].data[field] = value; } else { updatedContent[index].data = value; } setContent(updatedContent); };
    const removeContentBlock = (index) => { const updatedContent = content.filter((_, i) => i !== index); setContent(updatedContent); };

    // Handle form submission to update the document
    const handleSubmit = async (e) => {
        e.preventDefault();
        setIsLoading(true);
        const updatedTopic = { title, materialSlug, order: Number(order), content };
        ...
        try {
            ...
            alert('حدث خطأ أثناء تحديث الشرح.');
            setIsLoading(false);
        }
    };
    
    if (isLoading) return <p>جاري تحميل بيانات الشرح...</p>;

    return (
        <div>
            <h1 className="text-3xl font-bold mb-8">تعديل الشرح</h1>
            <form onSubmit={handleSubmit} className="space-y-8">
                {/* ... Basic Info Section (No changes) ... */}
                <div className="p-6 bg-surface-dark border border-border-color rounded-lg"> <h2 className="text-xl font-bold mb-4">المعلومات الأساسية</h2> <div className="space-y-4"> <div> <label htmlFor="title" className="block text-sm font-medium text-text-secondary mb-2">عنوان الشرح (باللغة الإنجليزية)</label> <input type="text" id="title" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded-lg border border-border-color bg-background-dark p-3" required /> </div> <div className="grid grid-cols-1 md:grid-cols-2 gap-6"> <div> <label htmlFor="material" className="block text-sm font-medium text-text-secondary mb-2">المادة التابع لها</label> <select id="material" value={materialSlug} onChange={(e) => setMaterialSlug(e.target.value)} className="w-full rounded-lg border border-border-color bg-background-dark p-3" required> <option value="">اختر مادة...</option> {allMaterials.map(mat => <option key={mat.id} value={mat.slug}>{mat.title}</option>)} </select> </div> <div> <label htmlFor="order" className="block text-sm font-medium text-text-secondary mb-2">الترتيب</label> <input type="number" id="order" value={order} onChange={(e) => setOrder(e.target.value)} className="w-full rounded-lg border border-border-color bg-background-dark p-3" required /> </div> </div> </div> </div>

                {/* Content Blocks Section (UPDATED) */}
