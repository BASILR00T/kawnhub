...
export default function NewTopicPage() {
    ...
    const addContentBlock = (type) => {
        ...
        setContent([...content, { id: uuidv4(), type, data: newBlockData }]);
    };

    // ---  THE FIX IS HERE (Corrected Logic) ---
    // We pass the block's unique ID to find it and update it
    ...
    const removeContentBlock = (blockId) => {
        setContent(prevContent => prevContent.filter(block => block.id !== blockId));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const newTopic = { title, materialSlug, order: Number(order), content: contentToSave, tags: tagSlugs };
