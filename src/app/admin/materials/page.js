...
export default function MaterialsPage() {
    ...
    // الخطوة 2: ننشئ دالة الحذف
    const handleDelete = async (id, title) => {
        // رسالة تأكيد بسيطة
        if (window.confirm(`هل أنت متأكد أنك تريد حذف المادة: "${title}"؟ لا يمكن التراجع عن هذا الإجراء.`)) {
            try {
                // نحذف المستند من Firebase
                await deleteDoc(doc(db, 'materials', id));
                // نحدّث القائمة في الواجهة فورًا بدون إعادة تحميل الصفحة
                            ...
                            <tr key={material.id}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div className="flex items-center justify-end gap-4">
                                        ...
                                        {/* الخطوة 3: نربط دالة الحذف بالزر */}
