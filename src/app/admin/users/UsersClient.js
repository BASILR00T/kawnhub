            ...
            <div className="bg-surface-dark border border-border-color rounded-2xl w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <form onSubmit={handleSave} className="p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-text-secondary mb-1">الاسم المعروض</label>
                            <input name="name" defaultValue={editingUser.name} placeholder="الاسم الكامل" className="w-full rounded-lg bg-background-dark border border-border-color p-3 text-sm focus:border-primary-blue outline-none" />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-text-secondary mb-1">التخصص</label>
                            <select name="major" defaultValue={editingUser.major || ''} className="w-full rounded-lg bg-background-dark border border-border-color p-3 text-sm focus:border-primary-blue outline-none">
                                <option value="">غير محدد</option>
                                <option value="CS">علوم حاسب (CS)</option>
                                <option value="IT">تقنية معلومات (IT)</option>
                                <option value="ISE">نظم (ISE)</option>
                                <option value="Common">سنة مشتركة</option>
                            </select>
                        </div>
                    </div>

                    <div className="pt-4 border-t border-border-color/50">
                        <label className="block text-xs font-bold text-text-secondary mb-2">الصلاحيات والرتبة</label>
